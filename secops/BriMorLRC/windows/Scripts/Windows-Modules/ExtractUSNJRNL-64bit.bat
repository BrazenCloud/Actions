...
GOTO TESTEXTRACTUSN
	...
	:TESTEXTRACTUSN	
		if exist "%TOOLSCRIPTPATH%ExtractUsnJrnl\ExtractUsnJrnl64.exe" (GOTO RUNEXTRACTUSN64) ELSE (GOTO SKIPPINGEXTRACTUSN64)
		...
		:RUNEXTRACTUSN64
			...
			@echo off
			echo. >> "%TRIMMEDSCRIPTPATH%%computername%%dt%\%computername%%dt%_Processing_Details.txt"
			echo Command Run: "%TOOLSCRIPTPATH%ExtractUsnJrnl\ExtractUsnJrnl64.exe" %SYSTEMDRIVE% "%TRIMMEDSCRIPTPATH%%computername%%dt%\LiveResponseData\CopiedFiles\usnjrnl" >> "%TRIMMEDSCRIPTPATH%%computername%%dt%\%computername%%dt%_Processing_Details.txt"
			@echo on
			"%TOOLSCRIPTPATH%ExtractUsnJrnl\ExtractUsnJrnl64.exe" %SYSTEMDRIVE% "%TRIMMEDSCRIPTPATH%%computername%%dt%\LiveResponseData\CopiedFiles\usnjrnl" >> "%TRIMMEDSCRIPTPATH%%computername%%dt%\%computername%%dt%_Processing_Details.txt" 2>&1
			
			@echo off
			GOTO ENDOFEXTRACTUSN64		
			
			:SKIPPINGEXTRACTUSN64
			@echo off
			echo.
			echo Skipping extraction of USNJournal File...
			echo ExtractUsnJrnl64.exe not found under the path "%TOOLSCRIPTPATH%ExtractUsnJrnl\ExtractUsnJrnl64.exe"
			echo Please download the program from https://github.com/jschicht/ExtractUsnJrnl
			echo and save the file to the folder "%TOOLSCRIPTPATH%ExtractUsnJrnl\ExtractUsnJrnl64.exe"
			echo.
			echo. >> "%TRIMMEDSCRIPTPATH%%computername%%dt%\%computername%%dt%_Processing_Details.txt"
			echo ExtractUsnJrnl64.exe not found under the path "%TOOLSCRIPTPATH%ExtractUsnJrnl\ExtractUsnJrnl64.exe" >> "%TRIMMEDSCRIPTPATH%%computername%%dt%\%computername%%dt%_Processing_Details.txt"
			...
			echo and save the file to the folder "%TOOLSCRIPTPATH%ExtractUsnJrnl\ExtractUsnJrnl64.exe" >> "%TRIMMEDSCRIPTPATH%%computername%%dt%\%computername%%dt%_Processing_Details.txt"					
			GOTO ENDOFEXTRACTUSN64
